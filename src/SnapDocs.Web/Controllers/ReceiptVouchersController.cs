...
public class ReceiptVouchersController : Controller
{
    ...
    public ReceiptVouchersController(SnapDocsDbContext db, IDocumentService documentService, IDocumentWorkflowService workflowService)
    {
        _db = db;
    ...
    public async Task<IActionResult> Index()
    {
        var documents = await _db.Documents
            ...
            .Where(x => x.CompanyId == DemoCompanyId && x.Type == DocumentType.ReceiptVoucher)
    ...
    {
        await LoadLookups();
        return View(new ReceiptVoucherCreateViewModel());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(ReceiptVoucherCreateViewModel model)
    {
        ...
        var dto = new CreateDocumentDto { CompanyId = DemoCompanyId, CustomerId = model.CustomerId, Type = DocumentType.ReceiptVoucher, DocumentDate = model.VoucherDate, DueDate = null, Discount = 0, TaxRate = 0, Notes = $"طريقة الدفع: {model.PaymentMethod} | المرجع: {model.ReferenceNumber} | {model.Notes}", Items = new List<DocumentItemInputDto> { new() { Description = "سند قبض", Quantity = 1, UnitPrice = model.Amount, Discount = 0 } } };
        ...
        TempData["Success"] = $"تم إنشاء سند القبض {document.Number} بنجاح";
    ...
    public async Task<IActionResult> Details(Guid id)
    {
        var document = await _db.Documents
            .Include(x => x.Customer)
            .Include(x => x.Items)
            .FirstOrDefaultAsync(x => x.Id == id && x.CompanyId == DemoCompanyId && x.Type == DocumentType.ReceiptVoucher);
    ...
    {
        var document = await _db.Documents.FirstOrDefaultAsync(x => x.Id == id && x.CompanyId == DemoCompanyId && x.Type == DocumentType.ReceiptVoucher);
        if (document is null) return NotFound();
