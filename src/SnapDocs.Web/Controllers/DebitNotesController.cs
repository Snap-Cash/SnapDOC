...
public class DebitNotesController : Controller
{
    ...
    public DebitNotesController(SnapDocsDbContext db, IDocumentService documentService, IDocumentWorkflowService workflowService)
    {
        _db = db;
    ...
    public async Task<IActionResult> Index(string? q, DocumentStatus? status)
    {
        var query = _db.Documents.Include(x => x.Customer)
            .Where(x => x.CompanyId == DemoCompanyId && x.Type == DocumentType.DebitNote);
    ...
    {
        await LoadLookups();
        return View(new DebitNoteCreateViewModel());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(DebitNoteCreateViewModel model)
    {
        ...
        TempData["Success"] = $"تم إنشاء الإشعار المدين {document.Number} بنجاح";
    ...
    public async Task<IActionResult> Details(Guid id)
    {
        var document = await _db.Documents.Include(x => x.Customer).Include(x => x.Items)
            .FirstOrDefaultAsync(x => x.Id == id && x.CompanyId == DemoCompanyId && x.Type == DocumentType.DebitNote);
    ...
    {
        var document = await _db.Documents.FirstOrDefaultAsync(x => x.Id == id && x.CompanyId == DemoCompanyId && x.Type == DocumentType.DebitNote);
        if (document is null) return NotFound();
